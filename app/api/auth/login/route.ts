...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user) {
      ...
      if (isPreviewEnvironment) {
        return NextResponse.json(
          {
            success: false,
            error: "Email not found in the system. Please try a different email or add this user first.",
          },
          { status: 404 },
        )
      }
      ...
      return NextResponse.json(
        { success: true, message: "If your email is in our system, you will receive a magic link" },
        { status: 200 },
      )
    }
    ...
    const result = await sendMagicLink(email, invitation.token, user.is_organizer)

    // In preview mode, return the magic link URL directly
    if (isPreviewEnvironment && result.magicLinkUrl) {
      return NextResponse.json(
        {
          success: true,
