...
export function WeChatGroupModal({
  ...
  joinedWeChat,
  onJoined,
}: WeChat & { onJoined: () => void }) {
  const [isOpen, setIsOpen] = useState(!joinedWeChat);
  const [isLoading, setIsLoading] = useState(false);
  const qrCodeUrl =
    process.env.NEXT_PUBLIC_WECHAT_QR_CODE ||
  ...
  async function handleJoinGroup() {
    ...
    try {
      ...
      toast.success("We've recorded that you've joined the WeChat group.");
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to update your status"
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Please scan this QR code to join our WeChat group for important
